...
int stack_Push (struct t_stack *name_stack, int element);
...
int stack_Plus (struct t_stack *name_stack, int element);
...
enum CANARY {
    ...
    CANARY_POSITION = 1, ///< позиция в стеке
    CANARY_VALUE = 666,  ///< значение канарейкпи
...
struct t_stack {
    ...
    int (*Push)(t_stack*, int) = stack_Push;
    
    int (*Pop)(t_stack*) = stack_Pop;
    
    int (*Peek)(t_stack*) = stack_Peek;
    ...
    int (*Plus)(t_stack*, int) = stack_Plus;
...
int main()
{
    ...
    t_stack mystack;
    mystack.BUG = mystack.Construct(&mystack, 2);
    mystack.Error_s(&mystack);
    
    mystack.BUG = mystack.Push(&mystack, 1);
    mystack.Error_s(&mystack);
    
    mystack.BUG = mystack.Push(&mystack, 2);
    mystack.Error_s(&mystack);
    
    mystack.BUG = mystack.Push(&mystack, 3);
    mystack.Error_s(&mystack);
    
    mystack.Pop(&mystack);
    
    mystack.BUG = mystack.Push(&mystack, 10);
    mystack.Error_s(&mystack);
